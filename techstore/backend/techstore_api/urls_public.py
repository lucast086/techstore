...
# API Routers para el esquema público
router = DefaultRouter()


# API root view para verificar estado del esquema público
@api_view(["GET"])
def public_api_root(request):
    """Endpoint principal para verificar estado de la API en el esquema público."""
    return Response(
        {
            "status": "online",
            "message": "API funcionando correctamente en esquema público",
            "version": "0.1.0",
        }
    )
...
urlpatterns = [
    ...
    # API URLs
    path("api/", public_api_root),  # Endpoint para verificar estado
