...
def upgrade():
    ...
    # Check if customer_accounts table already exists
    result = conn.execute(
        sa.text(
            "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customer_accounts');"
        )
    )
    customer_accounts_exists = result.scalar()

    if customer_accounts_exists:
        return
